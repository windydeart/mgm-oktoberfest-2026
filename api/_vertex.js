...
function getVertexCredentials() {
  const envVal = process.env.GCP_SERVICE_ACCOUNT_KEY ||
                 ...
                 process.env.VERTEX_AI_KEY;

  if (envVal) {
    try {
      let raw = envVal.trim();
      // Remove surrounding quotes if present (e.g. from Vercel UI copy/paste)
      ...
      if (parsed && parsed.client_email && parsed.private_key && parsed.project_id) {
        // Critical Vercel fix: convert literal '\\n' to actual newlines if escaped
      ...
      console.warn('Failed to parse GCP service account key from environment:', e.message);
    }
  }
  ...
  const localPaths = [
    ...
    join(__dirname, '..', 'api', 'gen-lang-client-0983914563-d15a149d75b0.json'),
...
async function callVertexGemini({
  ...
  generationConfig = {},
  timeoutMs = 3200,
  models = ['gemini-3.8-flash', 'gemini-3.6-flash', 'gemini-3.5-flash', 'gemini-2.5-flash', 'gemini-2.0-flash'],
